...
public partial class Boats : Node, Obstruction
{
	...
	Nest n;
	Node f;
	private static Dictionary<int, (int health, int damage, int speed)> BoatLevelStats = new Dictionary<int, (int health, int damage, int speed)>
	{
	...
	{
		(Health, Damage, Speed) = BoatLevelStats[level];
		Level = level;
	}
